...
def helpMessage() {
	...
	Options for R-DADA2:
	  ...
	  --nsamples [int]              Number of samples to select from DADA2 learning algorithm (process dada2errmodels).
	                                ...
	                                assuming you have at least 10 milion reads in total, fairly evenly distributedover samples.
	...
	*/
	...
	process dada2filter {
		...
		*/
		...
		"""
		dada2filter --verbose --trimleft=0,0 --trunclen=${trunclenR1},${trunclenR2} \
	...
	process dada2errmodels  {
		...
		file ("*.rds") into ch_fastq_dada2errmodels
	 	...
	 	script:
	 	"""
	 	dada2errmodels --verbose --nsamples=${params.nsamples} --maxconsist=${params.maxconsist} \
	...
	process dada2cleanNmerge  {
		...
		file ("*merged.rds") into ch_fastq_dada2cleanNmerge
	 	...
	 	script:
	 	"""
	 	dada2cleanNmerge --verbose ${params.concatenate} --filterdir=. \
	...
	process dada2bimeras  {
		...
		file ("*bimeras.tsv.gz") into ch_fastq_dada2bimeras
	 	...
	 	script:
	 	"""
	 	dada2bimeras --verbose --method=${params.method} --minab=${params.minab} --overab=${params.overab} \
