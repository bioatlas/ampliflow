...
if (params.rdp == true) {
	process dada2taxonomy_rdp {
		...
		file ("dada2taxonomy_log") into ch_dada2taxonomy_rdp_log
			...
			script:
		speciesDB = params.species ? '--species=${params.dbpath}${params.taxa_db}_species.fna' : ''
			"""
			dada2taxonomy.R --verbose --rdp_fna=${params.dbpath}${params.taxa_db}.fna sequences.fna.gz \
